...
class UpdateRsProxyTool(DataBaseTool):
  ...
  def executeMaya(self):
    ...
    project = db.queryOne("project", name="Payback_Amex")

    ws = self.workShop
    importCacheTool = ws.instantiate(cmd="importcache")

    refFolder = "E:/PROJECTS/PAY_Payback_Amex/Models/ref"
    sel = self.__sel
    ...
    for loc in sel:
        ...
        # Cleaning up locator with children
          ...
          for c in children:
            if cmds.referenceQuery(c, isNodeReferenced=True):
              ...
              if refFile.split("{")[0] != "E:/PROJECTS/PAY_Payback_Amex/Models/ref/Pointee_centerJnt.mb":
      ...
      cmds.setAttr(pxyNode + ".fileName", "E:/PROJECTS/PAY_Payback_Amex/Cache/rsProxy/Pointee_rsProxy_%s_####.rs" %(cName), type="string")
